...
(function () {
    ...
    const toText = (v) => {
      ...
      if (typeof v === "object") {
        if (Array.isArray(v.lines))
          return v.lines.map((x) => String(x == null ? "" : x)).join("\n");
        if (Array.isArray(v.value))
          return v.value.map((x) => String(x == null ? "" : x)).join("\n");
  ...
  const fillSelect = (sel, keys, MODELS) => {
    sel.innerHTML = keys
      .map((k) => `<option value="${esc(k)}">${esc(MODELS[k].name)}</option>`)
      .join("");
  };
  ...
  fetch(DATA_URL, { cache: "no-store" })
    ...
    .then((raw) => {
      ...
      const rowsSrc =
        raw && typeof raw === "object" && !Array.isArray(raw) && Array.isArray(raw.rows)
          ? raw.rows
          : raw && typeof raw === "object" && !Array.isArray(raw) && Array.isArray(raw.specRows)
            ? raw.specRows
      ...
      if (!specKeys.length && Array.isArray(rowsSrc) && rowsSrc.length) {
        ...
        rowsSrc.forEach((row) => {
          ...
          keys.forEach((id, idx) => {
            ...
            MODELS[id].specs.push({
              k,
              v: val == null || String(val).trim() === "" ? "-" : String(val)
            });
          });
        });
      }
