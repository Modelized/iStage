...
(function () {
  ...
  if (list) {
    list.innerHTML = [
      groupHTML(
        "Latest Releases",
        `<div class="empty small muted">Checking for latest updates...</div>`
      ),
      groupHTML(
        "Previous Releases",
        `<div class="empty small muted">Checking for previous updates...</div>`
      )
    ].join("");
  }
  if (!list) return;
  ...
  const mdToHtml = (md = "") => {
    ...
    for (let raw of lines) {
      ...
      const detailClass =
        inKnownIssues && knownIssueDetailPending ? ' class="known-issue-detail"' : "";
  ...
  fetch(`https://api.github.com/repos/${OWNER}/${REPO}/releases`, {
    ...
    .then((items) => {
      ...
      const latestReleases = Array.from(latestByVer.values()).sort(
      ...
      list.innerHTML = [
        ...
        group(
          "Previous Releases",
          previousReleases,
          previousReleases.length ? "" : "No previous releases."
        )
      ].join("");
    })
